...
var largestDivisibleSubset = function(nums) {
      nums = nums.sort(function(a,b){return b-a}); // sort for easier processing
      let solution = []; 
      
      if(nums.length===0) {return []} //catch invalid input
      ...
      function getNextDivisible(start, current, array){
            current++;
            if (current>array.length) return false;
            if(array[start]%array[current]===0) return current;
            return getNextDivisible(start, current, array)
            
      }
      
      function getAllDivisible(array){
            array = array.sort(function(a,b){return b-a}); // sort for easier processing
            let current = 0;
            let solution = [array[current]];
            
            
            while(getNextDivisible(0, current, array)!=false){
            current = getNextDivisible(0, current, array);
            solution.push(array[current]);
            }
            return solution;
      }

      while(nums.length>0){ //get all divisible subsets for each array item
            
            solution.push(getAllDivisible(nums));
            nums.splice(0,1);
      }
      
      console.log(solution[0]);

      console.log( deleteItem(solution[0]));

      // for(j=0;j<solution.length;j++){
      //       deleteItem(solution[j]);
      // }
